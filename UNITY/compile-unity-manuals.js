...
function getCss(accentColor, accentLight, bgCoverGrad, osBadge) {
  ...
  @page {
    ...
    @bottom-left {
      content: "${osBadge} — Manual de Unity y GitHub: De Novato a Avanzado";
      font-family: 'Inter', sans-serif;
      font-size: 8.5pt;
      color: #8c959f;
    }
...
<head>
  <meta charset="UTF-8">
  <title>${title.replace(/<[^>]*>?/gm, '')} - ${osBadge}</title>
  <style>${css}</style>
</head>
<body>
  <div class="cover-page">
    <div class="cover-top">
      ...
      <span class="version-tag">Unity 2022 LTS / Unity 6 • Git LFS • 2026</span>
    </div>
    <div class="cover-center">
      <h1 class="cover-title">${title}</h1>
      <p class="cover-subtitle">${subtitle}</p>
      <div class="cover-features">
        <span class="feature-chip">Git LFS & Assets Binarios</span>
        <span class="feature-chip">UnityYAMLMerge 3-Way Driver</span>
        <span class="feature-chip">Visible Meta Files & GUIDs</span>
        <span class="feature-chip">Multi-Scene & Prefab Isolation</span>
        <span class="feature-chip">GameCI & GitHub Actions</span>
        <span class="feature-chip">Catálogo de Incidentes Críticos</span>
      </div>
    </div>
    <div class="cover-footer">
      <div class="author-info">
        <strong>Manual de Referencia y Práctica en Videojuegos</strong>
        Guía de Novato a Avanzado • Unity & GitHub Engine Workflow
      </div>
      <div class="doc-meta">
        Edición Técnica Profesional<br>
        Formato de Alta Calidad para Producción
      </div>
    </div>
  </div>
...
function compileManual({ mdPath, pdfPath, osBadge, accentColor, accentLight, bgCoverGrad, title, subtitle }) {
  ...
  console.log(`✓ PDF generado exitosamente (${(stats.size / 1024).toFixed(1)} KB)`);
}

const unityDir = __dirname;

const unityManuals = [
  {
    mdPath: path.join(unityDir, 'manual-unity-github-debian-linux.md'),
    pdfPath: path.join(unityDir, 'manual-unity-github-debian-linux.pdf'),
    osBadge: 'Debian GNU/Linux',
    accentColor: '#d70a53',
    accentLight: '#ff6584',
    bgCoverGrad: 'linear-gradient(135deg, #0d1117 0%, #1e0d16 45%, #380718 100%)',
    title: 'MANUAL DE UNITY Y GITHUB<br><span>DE NOVATO A AVANZADO</span>',
    subtitle: 'Guía exhaustiva para el desarrollo colaborativo de videojuegos en Debian GNU/Linux con Bash, Unity Hub, Git LFS, UnityYAMLMerge y GameCI'
  },
  {
    mdPath: path.join(unityDir, 'manual-unity-github-powershell-windows.md'),
    pdfPath: path.join(unityDir, 'manual-unity-github-powershell-windows.pdf'),
    osBadge: 'Windows PowerShell',
    accentColor: '#0078d4',
    accentLight: '#50e6ff',
    bgCoverGrad: 'linear-gradient(135deg, #091322 0%, #0b2240 45%, #003a70 100%)',
    title: 'MANUAL DE UNITY Y GITHUB<br><span>DE NOVATO A AVANZADO</span>',
    subtitle: 'Guía exhaustiva para el desarrollo colaborativo de videojuegos en Microsoft Windows con PowerShell 7+, Git Credential Manager, UnityYAMLMerge y GameCI'
  },
  {
    mdPath: path.join(unityDir, 'manual-unity-github-macos-apple-silicon.md'),
    pdfPath: path.join(unityDir, 'manual-unity-github-macos-apple-silicon.pdf'),
    osBadge: 'macOS Apple Silicon',
    accentColor: '#6366f1',
    accentLight: '#a5b4fc',
    bgCoverGrad: 'linear-gradient(135deg, #0f1117 0%, #15162b 45%, #1e1b4b 100%)',
    title: 'MANUAL DE UNITY Y GITHUB<br><span>DE NOVATO A AVANZADO</span>',
    subtitle: 'Guía exhaustiva para el desarrollo colaborativo de videojuegos en macOS con Apple Silicon (M1/M2/M3/M4), Zsh, Keychain, UnityYAMLMerge y GameCI'
  }
];

console.log('Iniciando compilación de los manuales de Unity a PDF...');
for (const m of unityManuals) {
  compileManual(m);
}
console.log('\n¡Todos los manuales PDF de Unity han sido generados exitosamente!');
